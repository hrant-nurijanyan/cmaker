...
if args.template is not None:
    if args.template not in templates:
        raise ValueError(f'Invalid project template type, it must be one of {templates} but is {args.template}')
else:
    args.template = 'clean'
...
template_dir = str(Path('~').expanduser()) + f"/.local/share/cmaker/templates/cpp_{template}"
...
with open('CMakeLists.txt','r') as CMakeLists:
    ...
    content = content.replace("{min_cmake_version}",f'VERSION {cmake_version}')
    content = content.replace("{project_name}",name)
    content = content.replace("{cxx_standard}",standard)
    content = content.replace("{project_type}","library") if isLib else content.replace("{project_type}","executable")
